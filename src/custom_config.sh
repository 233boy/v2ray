...
if [[ -f /usr/local/etc/v2ray/custom/in.json ]]; then
	custom_config_file="/usr/local/etc/v2ray/custom/in.json"
...
if [[ -f /usr/local/etc/v2ray/custom/out.json ]]; then
	custom_config_file="/usr/local/etc/v2ray/custom/out.json"
	sed -i "/\/\/include_out_config/r $custom_config_file" $v2ray_server_config
	sed -i "s#//include_out_config#,#" $v2ray_server_config
fi
# custom rules
if [[ -f /usr/local/etc/v2ray/custom/rules.json ]]; then
	custom_rules_file="/usr/local/etc/v2ray/custom/rules.json"
